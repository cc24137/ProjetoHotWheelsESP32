...
class Max7219(framebuf.FrameBuffer):
    ...
    def scroll_text(self, message, delay=0.05, repeat=1):
        # Calcula a largura total do texto (cada caractere tem 8 pixels)
        text_width = len(message) * 8
        width = self.width
        # Pega a largura do display definida na inicialização
        for _ in range(repeat):
            # Vai de x=largura (fora da tela à direita) até sumir à esquerda
            for x in range(width, -text_width, -1):
                self.fill(0)
                self.text(message, x, 0, 1)
                self.show()
                time.sleep(delay)