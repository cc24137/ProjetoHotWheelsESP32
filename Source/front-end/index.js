...
function mudarIP() {
    ...
    if (!inputUrl.startsWith('http')) baseUrl = "http://" + inputUrl;
...
function updateGauge(speed, max = 5) {
    ...
    gaugeText.textContent = speed.toFixed(2);
}

function updateUI() {
    const cur = readings.length ? readings[readings.length - 1].speed : null;

    document.getElementById('currentSpeed').innerHTML = cur !== null
        ? `${cur.toFixed(2)}<span class="unit">m/s</span>`
        : `—<span class="unit">m/s</span>`;

    document.getElementById('topSpeed').innerHTML = topSpeed > 0
        ? `${topSpeed.toFixed(2)}<span class="unit">m/s</span>`
        : `—<span class="unit">m/s</span>`;
    ...
    document.getElementById('tickCurrent').textContent = cur !== null ? `${cur.toFixed(2)} m/s` : '— m/s';
    document.getElementById('tickPeak').textContent = topSpeed > 0 ? `${topSpeed.toFixed(2)} m/s` : '— m/s';
